...
`,
	...
	cmdDumpSql := &cobra.Command{
		Use:   "dump-sql",
		Short: "Generate a SQL file which contains all migrations",
		Long: `Generate a SQL file which contains all migrations

This generates a single SQL file containing all migrations required to update
the database to the latest schema version, starting from the initial migration.
...
against your database, as it does not update the version table.

Limitations:

All migrations, from the first to the latest, are included in the file.
`,
		Run: DumpSql,
	}
	...
	cmdDumpSql.Flags().StringSliceVarP(&cliOptions.configPaths, "config", "c", []string{}, "config path (default is ./tern.conf)")
	cmdDumpSql.Flags().StringVarP(&cliOptions.versionTable, "version-table", "", "", "version table name (default is public.schema_version)")
	cmdDumpSql.Flags().StringVarP(&cliOptions.currentVersion, "current", "", "0", "current version of the database")
	cmdDumpSql.Flags().StringVarP(&cliOptions.destinationVersion, "destination", "d", "last", "destination migration version")

	cmdDumpSql.Flags().StringVarP(&cliOptions.migrationsPath, "migrations", "m", "", "migrations path (default is .)")
	cmdDumpSql.Flags().StringVarP(&cliOptions.gengenOutputFile, "output", "o", "", "output file")
...
`))
	...
	writeSql(gengenTemplate)
}

func writeSql(gengenTemplate *template.Template) {
	config, err := LoadConfig()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error loading config:\n  %v\n", err)
		os.Exit(1)
...
func DumpSql(cmd *cobra.Command, args []string) {

	dumpSqlTemplate := template.Must(template.New("dump-sql").Parse(`-- This file was generated by tern dump-sql v{{ .Version }}.
...
`))
	...
	destinationArg := cliOptions.destinationVersion

	mustParseCurrent := func(d string) int32 {
		var n int64
		n, err = strconv.ParseInt(d, 10, 32)
		if err != nil {
	...
	currentVersion := mustParseCurrent(cliOptions.currentVersion)
	maxDestination := int32(len(migrator.Migrations))

	destination := parseDestination(destinationArg, currentVersion, maxDestination)

	plan, err := migrator.PlanMigrate(currentVersion, destination)
	if err != nil {
	...
	err = dumpSqlTemplate.Execute(out, map[string]any{
		"Version":      VERSION,
		"VersionTable": config.VersionTable,
...
func parseDestination(destinationArg string, currentVersion int32, maxDestination int32) int32 {
	mustParseDestination := func(d string) int32 {
		var n int64
		n, err := strconv.ParseInt(d, 10, 32)
		if err != nil {
	...
	} else if len(destinationArg) >= 3 && destinationArg[0:2] == "-+" {
		destination = currentVersion - mustParseDestination(destinationArg[2:])
